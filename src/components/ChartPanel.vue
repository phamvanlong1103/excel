<template>
  <div class="bg-white border-r border-gray-200 flex flex-col"
    :style="{ minWidth: '200px', maxWidth: '400px', width: width + 'px' }">
    ...
    <div class="p-4 border-b border-gray-200">
      ...
      <div class="grid grid-cols-2 gap-2">
        <button v-for="type in chartTypes" :key="type.value" @click="$emit('update:selectedChartType', type.value)"
          :draggable="true" @dragstart="onChartTypeDragStart(type.value, $event)" :class="[
    ...
    <div v-if="selectedChartType" class="p-4 border-t border-gray-200 flex-1 overflow-y-auto">
      ...
      <div v-if="selectedDataSources.length === 0 && !alwaysShowProperties"
      ...
      <div v-else class="space-y-3">
        <div>
          ...
          <input v-model="chartConfig.title" type="text" placeholder="Enter chart title"
        ...
        <div v-if="selectedChartType === 'pie'">
          ...
          <div @drop="$emit('field-drop', $event, 'category')" @dragover.prevent @dragenter.prevent
        ...
        <div v-else-if="selectedChartType === 'bar'">
          ...
          <div @drop="$emit('field-drop', $event, 'xAxis')" @dragover.prevent @dragenter.prevent
            ...
            <template v-if="Array.isArray(chartConfig.xAxis) && chartConfig.xAxis.length > 0">
              <span v-for="(field, idx) in chartConfig.xAxis" :key="field"
                class="inline-flex items-center px-2 py-1 bg-primary-100 text-primary-800 rounded mr-1">
                {{ field }}
                <button @click.stop="$emit('remove-x-axis', idx)"
                  class="ml-1 text-xs text-primary-700 hover:text-red-500">&times;</button>
              </span>
            </template>
          ...
          <div class="mt-2">
            ...
            <div @drop="$emit('field-drop', $event, 'yAxis')" @dragover.prevent @dragenter.prevent
              class="min-h-[2.5rem] p-2 border-2 border-dashed border-gray-300 rounded text-sm text-gray-500 flex items-center justify-center hover:border-primary-400 transition-colors duration-200"
        ...
        <div v-else-if="selectedChartType === 'bubble'">
          <!-- Trục X -->
          <!-- Trục X -->
          ...
          <div @drop="$emit('field-drop', $event, 'xAxis')" @dragover.prevent @dragenter.prevent
            class="min-h-[2.5rem] p-2 border-2 border-dashed border-gray-300 rounded text-sm text-gray-500 flex items-center justify-center hover:border-primary-400 transition-colors duration-200"
            :class="{ 'border-primary-400 bg-primary-50': Array.isArray(chartConfig.xAxis) && chartConfig.xAxis.length > 0 }">
            {{ chartConfig.xAxis?.[0] || 'Drop X-axis field here' }}
          </div>
          ...
          <div @drop="$emit('field-drop', $event, 'yAxis')" @dragover.prevent @dragenter.prevent
            class="min-h-[2.5rem] p-2 border-2 border-dashed border-gray-300 rounded text-sm text-gray-500 flex items-center justify-center hover:border-primary-400 transition-colors duration-200"
            :class="{ 'border-primary-400 bg-primary-50': chartConfig.yAxis }">
            {{ chartConfig.yAxis || 'Drop Y-axis field here (numeric)' }}
          </div>

          <!-- Kích thước Bubble -->
          <label class="block text-xs font-medium text-gray-600 mb-1">Bubble Size</label>
          <div @drop="$emit('field-drop', $event, 'category')" @dragover.prevent @dragenter.prevent
            class="min-h-[2.5rem] p-2 border-2 border-dashed border-gray-300 rounded text-sm text-gray-500 flex items-center justify-center hover:border-primary-400 transition-colors duration-200"
            :class="{ 'border-primary-400 bg-primary-50': chartConfig.category }">
            {{ chartConfig.category || 'Drop bubble size field here (numeric)' }}
          </div>
        ...
        <div v-if="selectedChartType === 'bar'" class="mt-2">
          ...
          <div class="flex items-center gap-1 mt-2">
            <span
              v-for="(color, idx) in colorPalettes[chartConfig.colorScheme] && colorPalettes[chartConfig.colorScheme].slice(0, 8)"
              :key="color + idx" class="w-5 h-5 rounded-full border border-gray-200"
        ...
        <div class="grid grid-cols-2 gap-2">
          ...
          <div>
            ...
            <input v-model="chartConfig.borderColor" type="color"
              class="w-full h-8 rounded border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500" />
          </div>
        </div>

        <button @click="$emit('add-or-update-chart')" :disabled="!isChartConfigValid"
        ...
        <button v-if="editingChartId" @click="$emit('cancel-edit')"
          class="w-full mt-2 inline-flex items-center justify-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">Cancel
        </button>
      </div>
    </div>
  </div>
